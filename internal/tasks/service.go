...
func (s *Service) CreateFor(ctx context.Context, actor Actor, input CreateInput) (Task, error) {
	title, notes, err := validateText(input.Title, input.Notes)
	if err != nil {
		return Task{}, err
	}
	if input.Visibility == "" {
		input.Visibility = VisibilityPersonal
	}
	if input.Visibility != VisibilityPersonal && input.Visibility != VisibilityShared {
		return Task{}, fmt.Errorf("%w: visibility must be personal or shared", ErrInvalidInput)
	}
	now := s.now().UTC()
	owner, creator := actor.ID, actor.ID
	task := Task{Title: title, Notes: notes, Status: StatusPending, SourceType: SourceManual, CreatedAt: now, UpdatedAt: now, OwnerID: &owner, CreatedBy: &creator, Visibility: input.Visibility, CanManage: true, Owner: &OwnerSummary{ID: actor.ID}}
	if err := s.repo.CreateOwned(ctx, &task); err != nil {
		return Task{}, err
	}
	return task, nil
}

func (s *Service) GetFor(ctx context.Context, actor Actor, id int64) (Task, error) {
	if id <= 0 {
		return Task{}, ErrNotFound
	}
	return s.repo.GetVisible(ctx, actor, id)
}

func (s *Service) ListFor(ctx context.Context, actor Actor, options ListOptions) (ListResult, error) {
	if options.Status == "" {
		options.Status = "all"
	}
	...
	items, total, err := s.repo.ListVisible(ctx, actor, options)
	if err != nil {
...
func (s *Service) UpdateFor(ctx context.Context, actor Actor, id int64, input UpdateInput) (Task, error) {
	...
	task, err := s.GetFor(ctx, actor, id)
	if err != nil {
		return Task{}, err
	}
	if !task.CanManage {
...
func (s *Service) DeleteFor(ctx context.Context, actor Actor, id int64) error {
	task, err := s.GetFor(ctx, actor, id)
	if err != nil {
		return err
	}
