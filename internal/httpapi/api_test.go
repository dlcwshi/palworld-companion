...
func newFixtureWithRosterTTL(t *testing.T, client palworld.Client, rosterTTL time.Duration) *fixture {
	...
	return &fixture{handler: New(status, tasks.NewService(tasks.NewRepository(db.SQL())), service, BuildInfo{Name: "Palworld Companion", Version: "0.4.5-dev"}, logger, assets), db: db, logs: logs}
}
