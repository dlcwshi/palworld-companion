...
## 2026-07-16 v0.4.6-dev server overview deployment record

- Source commit: `56788a5f98aa58c91c7e24735b89421310b553f9`; version: `0.4.6-dev`; UTC build time: `2026-07-16T13:04:44Z`. The pure-Go Linux AMD64 artifact was built with `CGO_ENABLED=0`, `GOOS=linux`, and `GOARCH=amd64`; size is `11301048` bytes and SHA-256 is `6df940ab227c0f3726d9b687d29e5d10e0f04c9f43cb314dbabede8a29d6cf23`. The verified upload remains `/tmp/palworld-companion-56788a5`.
- Embedded production assets are `assets/index-C-Pr4XZT.js` (`139273` bytes, SHA-256 `f5745bf06f13ecc692eafe37d9d0cd7c88e4d78f5f59442fcf424579cb2ba7bb`), `assets/index-DG-6jrti.css` (`25648` bytes, SHA-256 `32f1e1ee6aa4963580ac8bcaaf1741b7448a7a2d2506998fa3618c695a2bad6e`), `assets/workbox-window.prod.es5-BBnX5xw4.js` (`5748` bytes, SHA-256 `cadd21be4a88598a4dec91a26a025df0a5790ff6baec54a884e401c3f5c95557`), and `sw.js` (`1189` bytes, SHA-256 `cc870de833c03bd2a2951b9de63969e1bd93c6657d063c57a83f80e6fdcbbad2`). Production downloads matched all local hashes. PWA manifest and Service Worker behavior were not changed.
- The home page now renders one `server-overview-card`: status and version, a stable name/online grid, and four divider-separated metrics in a single row. It no longer renders the four independent `MetricCard` frames or a separate metrics grid. The server description remains absent from the home DOM; the summary API and backend field remain unchanged. At widths of at least 360px the metric area is `repeat(4, minmax(0, 1fr))`; only narrower screens use the 2x2 fallback.
- The saved v0.4.5 390px acceptance measured the old status card at `134.47px`; its separate two-row metric grid was `193px` with a `10px` gap, so the full split overview envelope occupied approximately `337.47px`. The v0.4.6 single card is `171.25px`, reducing the overview envelope by approximately `166.22px` (about 49%). The task-title top moved from `460.55px` to `279.33px` without hiding content or adding horizontal overflow.
- Final browser measurements for card height / task-title top were: 390x844 `171.25 / 279.33px`, 412x915 `175.67 / 285.33px`, 430x932 `179.28 / 289.28px`, 768x1024 `186 / 368px`, and 950x690 `186 / 368px`. Every viewport rendered one overview container, four metric cells in four columns, zero description nodes, zero horizontal overflow, and zero console errors. The four summary metric cards remained removed; the four task/player summary behavior and bottom navigation were unchanged.
- A separate real Vue stress run used a long server name, `50 / 50`, `999h 59m`, world day `9999`, and base count `99`. At 390x844 the card remained `171.25px`, the name clamped to two lines, all four metric values remained complete in four columns, and the title, online count, metric values, and page had no horizontal overflow. The account page still rendered zero password inputs/panel by default and the five bottom navigation items still rendered five SVG icons.
- Frontend verification passed `npm ci`, `npm audit` with 0 vulnerabilities, type-check, ESLint, home grouping, post-login redirect, UI contracts, production build, and build verification. `go test -count=1 ./...` passed before and after the final `frontend/dist` rebuild, and `go vet ./...` passed.
- Deployment backup: `/root/palworld-companion-backup-20260716-130737`. It contains the prior binary, unchanged configuration, unit, and `companion.db` copied after Companion stopped. WAL/SHM existed before shutdown (`4148872` and `32768` bytes) but were cleanly checkpointed and absent after shutdown, so no sidecar files were copied. No rollback occurred.
- `/etc/palworld-companion/config.yaml` was not modified; SHA-256 stayed `194a975a1e55fa0be3e6d3c5f9e1a3239abbce4dff1d8302752287b7989df58e`. SQLite stayed at schema 5 in WAL mode with `foreign_key_check=0` and `setupRequired=false`. Before and after deployment counts remained `users=3`, `sessions=10`, `tasks=3`, `auth_flows=0`, and `player_roster=2`; no business data or setup state was lost.
- `palworld-companion.service` remained enabled, active, and running. PID changed from `150300` (active since `2026-07-16 10:28:40 UTC`) to `152299` (active since `2026-07-16 13:07:37 UTC`), and health returned `0.4.6-dev`. HTTP continued to redirect 301 to HTTPS; HTTPS `/` and `/login` returned 200; summary and players returned 200; unauthenticated tasks returned 401; both legacy Steam routes returned 410. The deployed bundle contained the single overview contract and no home `metrics-grid` or server-description text. Post-deployment log error count was zero.
- Only `palworld-companion.service` was stopped and started. `palworld-server.service` remained PID `113468`, active since `2026-07-15 09:35:34 UTC`; `palworld-pst.service` remained PID `107527`, active since `2026-07-15 01:48:58 UTC`. Nginx, FRP, firewall, port 8212 configuration, Palworld/PST configuration or data, player saves, and SQLite schema were not modified. Docker and `clean-seeds` were not used. No Git tag or GitHub release was created.
- Remaining real-person acceptance: reopen the installed PWA on a physical phone and confirm the update to 0.4.6-dev and the compact overview density; then repeat with the real server name, maximum observed online count, a second real player joining/leaving, and Android gesture safe-area behavior. No production test account or password was created or entered during this deployment.
