...
SQLite 默认位于 `/var/lib/palworld-companion/companion.db`，使用 WAL、外键约束和 5 秒 busy timeout。迁移版本 1 创建 `tasks` 表；业务数据不依赖 PST 或 Palworld 存档。Palworld API 暂时不可用时，任务 API 仍可独立工作。
