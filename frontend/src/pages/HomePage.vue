...
const status = computed(() => {
  ...
  if (store.summary.stale) return { label: '显示缓存数据', tone: 'warning' }
...
onMounted(() => {
  void auth.refresh().then(async () => {
    if (auth.authenticated) {
      const [mine, shared] = await Promise.all([api.tasks.list('pending', 5, 'mine'), api.tasks.list('pending', 5, 'shared')])
      mineTasks.value = mine.tasks
      sharedTasks.value = shared.tasks
      void taskStore.load('pending', 5, 'visible')
    }
  })
  void store.refresh()
...
<template>
  <div class="home-page">
    <header class="hero-header">
      <div><p class="eyebrow">SELF-HOSTED · V0.4 DEV</p><h1>Palworld<br /><em>Companion</em></h1></div>
    ...
    <div v-if="isOffline" class="notice warning">浏览器当前离线，正在显示上一次加载的页面。</div>
    <div v-if="store.backendError" class="notice danger">{{ store.backendError }}。已保留上次数据显示。</div>
    <div v-else-if="store.summary?.error" class="notice danger">服务器状态暂时无法确认，以下为上次成功数据。</div>
