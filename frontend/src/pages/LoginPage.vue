...
async function submit() { busy.value = true; error.value = null; try { await auth.login(account.value, password.value); await router.replace(resolvePostLoginPath(route.query.returnTo)) } catch(value) { error.value = value instanceof APIError ? messages[value.code] ?? value.message : '登录失败，请重试。' } finally { busy.value = false } }
</script>
