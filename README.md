...
**当前仓库版本：0.5.0-dev。**不创建 v0.5.0 Tag 或正式 Release。
...
登录成功默认进入首页 `/`。只有用户明确访问 `/tasks` 或管理员用户页时才保留站内回跳；`/account`、登录/注册/初始化页面、未知路径和外部 URL 一律回到首页，避免 PWA 恢复账户页后再次登录仍停留在修改密码页面。
