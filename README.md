...
**当前仓库版本：0.4.1-dev。**不创建 v0.4.1 Tag 或正式 Release。
...
- 个人任务与共享任务，权限在 SQL 和 service 两层校验；首页按互斥分组去重展示。
- 移动端 PWA 自动更新、SQLite WAL、纯 Go Linux AMD64 单二进制和 systemd 部署。
...
- 首页只从一次 `scope=visible,status=pending` 结果派生任务：先按稳定 ID 去重，再将当前用户的 personal 任务与全部 shared 任务互斥分组；共享任务不会重复进入个人分组。
...
配置中的 `auth.enabled`、`public_base_url`、`admin_steam_ids` 仅为旧版本兼容字段，0.4.1-dev 会读取但不会用于 Steam 认证。仍使用 `auth.session_ttl` 控制 Session 有效期。

## 持久化玩家名册

0.4.1-dev 将完整校验后的新鲜 /players 成功快照写入 SQLite schema 5 的 player_roster。稳定身份键是内部 palworld_user_id，角色名只用于显示和本地登录查找；公共 API 不返回 SteamID64、Palworld userId/playerId、accountName、IP 或数据库 ID。
